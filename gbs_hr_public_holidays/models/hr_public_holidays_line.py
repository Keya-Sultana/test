...
class HrPublicHolidaysLine(models.Model):
    ...
    @api.depends('public_type_id', 'weekly_type_id')
    def _compute_year_id(self):
        for line in self:
            if line.public_type_id:
                line.year_id = line.public_type_id.id
            elif line.weekly_type_id:
                line.year_id = line.weekly_type_id.id
            else:
                line.year_id = False
    ...
    year_id = fields.Integer("Calendar Year", compute="_compute_year_id", readonly=True, store=True)
