...
class SSLCOMMERZController(http.Controller):
    ...
    def sslcommerz_dpn(self, **data):
        ...
        try:
            ...
            if data:
                request.env['payment.transaction'].sudo()._handle_feedback_data('sslcommerz', data)
            else:
    ...
    def _validate_data_authenticity(self, **data):
        ...
        The verification is done in three steps:
          - 1: POST the complete, unaltered, message back to SSLCOMMERZ (preceded by
               `cmd=_notify-validate`), in the same encoding.
          - 2: SSLCOMMERZ sends back either 'VERIFIED' or 'INVALID'.
        ...
        In practice, however, the transaction ID is never given by SSLCOMMERZ and the documentation
        ...
        # Request SSLCOMMERZ for an authenticity check
        data['cmd'] = '_notify-validate'
        response = requests.post(acquirer_sudo._sslcommerz_get_api_url(), data, timeout=60)
        ...
        if response_code == 'VERIFIED':
            ...
            if response_code == 'INVALID':
                error_message = "SSLCOMMERZ: " + _("Notification data were not acknowledged.")
            else:
                error_message = "SSLCOMMERZ: " + _(
