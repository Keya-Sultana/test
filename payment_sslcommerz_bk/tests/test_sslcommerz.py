...
from .common import SSLCOMMERZCommon
from ..controllers.main import SSLCOMMERZController


@tagged('post_install', '-at_install')
class SSLCOMMERZForm(SSLCOMMERZCommon):

    def _get_expected_values(self):
        return_url = self._build_url(SSLCOMMERZController._return_url)
        values = {
            'address1': 'Huge Street 2/543',
            'amount': str(self.amount),
            'business': self.sslcommerz.sslcommerz_email_account,
            ...
            'first_name': 'Norbert',
            'item_name': f'{self.sslcommerz.company_id.name}: {self.reference}',
            'item_number': self.reference,
            'last_name': 'Buyer',
            'lc': 'en_US',
            'notify_url': self._build_url(SSLCOMMERZController._notify_url),
            'return': return_url,
            'rm': '2',
            'zip': '1000',
        }

        if self.sslcommerz.fees_active:
            fees = self.currency.round(self.sslcommerz._compute_fees(self.amount, self.currency, self.partner.country_id))
            if fees:
    ...
    def test_redirect_form_values(self):
        ...
        self.assertDictEqual(
            ...
            "SSLCOMMERZ: invalid inputs specified in the redirect form.")
    ...
    def test_feedback_processing(self):
        ...
        with self.assertRaises(ValidationError):
            self.env['payment.transaction']._handle_feedback_data('sslcommerz', sslcommerz_post_data)
    ...
    def test_fees_computation(self):
        ...
        #In this way, SSLCOMMERZ will take 103.30 * 2.9% + 0.30 = 3.30€
        ...
        total_fee = self.sslcommerz._compute_fees(100, False, False)
        self.assertEqual(round(total_fee, 2), 3.3, 'Wrong computation of the SSLCOMMERZ fees')
