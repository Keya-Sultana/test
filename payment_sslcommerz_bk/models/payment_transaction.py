...
from odoo.addons.payment_sslcommerz.const import PAYMENT_STATUS_MAPPING
...
class PaymentTransaction(models.Model):
    ...
    sslcommerz_type = fields.Char(
        string="SSLCOMMERZ Transaction Type", help="This has no use in Odoo except for debugging.")
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        notify_url = self.acquirer_id.sslcommerz_use_ipn \
                     and urls.url_join(base_url, SSLCOMMERZController._notify_url)
        return {
            'store_id': 'sinda61744e05953ff',
            'address1': self.partner_address,
            'amount': self.amount,
            # 'business': self.acquirer_id.sslcommerz_email_account,
            ...
            'api_url': self.acquirer_id._sslcommerz_get_api_url(),
        }

    @api.model
    def _get_tx_from_feedback_data(self, provider, data):
        """ Override of payment to find the transaction based on SSLCOMMERZ data.
        ...
        tx = self.search([('reference', '=', reference), ('provider', '=', 'sslcommerz')])
        if not tx:
            raise ValidationError(
                "SSLCOMMERZ: " + _("No transaction found matching reference %s.", reference)
            )
        return tx

    def _process_feedback_data(self, data):
        """ Override of payment to process the transaction based on SSLCOMMERZ data.
        ...
        self.sslcommerz_type = txn_type

        payment_status = data.get('payment_status')

        if payment_status in PAYMENT_STATUS_MAPPING['pending'] + PAYMENT_STATUS_MAPPING['done'] \
            and not (self.acquirer_id.sslcommerz_pdt_token and self.acquirer_id.sslcommerz_seller_account):
            # If a payment is made on an account waiting for configuration, send a reminder email
            self.acquirer_id._sslcommerz_send_configuration_reminder()
        ...
        if payment_status in PAYMENT_STATUS_MAPPING['pending']:
            ...
            self._set_error(
                "SSLCOMMERZ: " + _("Received data with invalid payment status: %s", payment_status)
            )
