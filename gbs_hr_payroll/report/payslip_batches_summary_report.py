...
class PayslipBatchesSummaryReport(models.AbstractModel):
    _name = 'report.gbs_hr_payroll.report_payslip_batches_summary'
    ...
    def _get_report_values(self, docids, data):
        ...
        docs = self.env['hr.payslip.run'].search([('date_start', '<=', start_date),
                                                  ('date_end', '>=', end_date)])
                ...
                if report_type == 'cash':
                    if not slip.employee_id.bank_account_id.id:
                        if d.id == slip.employee_id.department_id.id:
                            payslip['emp_name'] = slip.employee_id.display_name
        ...
        for other_slip in docs.slip_ids:
            if not other_slip.employee_id.department_id.id:
                ...
                payslip = {}
                payslip['sn'] = sn
                payslip['emp_name'] = other_slip.employee_id.display_name
        ...
        amt_to_word = docs.company_id.currency_id.amount_to_text(all_total)
        print(amt_to_word)
