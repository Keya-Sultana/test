...
class AssessmentChecklistItem(models.Model):
    _name = 'assessment.checklist.item'
    _rec_name = 'line_name'

    line_name = fields.Char(string='Name', required=True)
    user_id = fields.Many2one('res.users', string='Responsible User', required=True)
    marking = fields.Integer(string='Out Of', default=10)


class AssessmentChecklistLine(models.Model):
    _name = 'assessment.checklist.line'
    _rec_name = 'name'
