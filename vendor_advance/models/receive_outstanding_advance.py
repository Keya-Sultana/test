...
class ReceiveOutstandingAdvance(models.Model):
    ...
    name = fields.Char('Name', required=True, tracking=True, copy=False, default='/')
    ...
    advance_ids = fields.Many2many('vendor.advance', string='Advance', tracking=True, readonly=True,
                                   required=True, states={'draft': [('readonly', False)]}, copy=False,
                                   domain=[('active', '=', False)])
    amount = fields.Float('Amount', required=True, tracking=True, readonly=True,
    ...
    maker_id = fields.Many2one('res.users', 'Maker', default=lambda self: self.env.user.id, tracking=True,
                               copy=False)
