...
class VendorAdvance(models.Model):
    ...
    name = fields.Char(required=True, copy=False, tracking=True, string='Name', default='/')
    date = fields.Date(string='Date ', required=True, default=fields.Date.today(), tracking=True,
                       readonly=True, states={'draft': [('readonly', False)]})
    partner_id = fields.Many2one('res.partner', string='Partner', ondelete='restrict', required=False, readonly=True,
                                 tracking=True, domain=[('parent_id', '=', False), ('supplier_rank','>', 0)], states={'draft': [('readonly', False)]})
    ...
    advance_amount = fields.Float(string="Approved Advance", required=True, readonly=True,
                                  ...
                                  tracking=True, help="Finally granted advance amount.")
    adjusted_amount = fields.Float(string="Adjusted Amount", tracking=True, copy=False,
    ...
    outstanding_amount = fields.Float(string="Outstanding Amount", compute="_compute_outstanding_amount", copy=False,
                                      tracking=True, help="Remaining Amount to adjustment.", store=True)
    account_id = fields.Many2one('account.account', string="GL Account", required=True, readonly=True,
                                 tracking=True, states={'draft': [('readonly', False)]},
                                 help="Account for the advance.")
    description = fields.Char('Particulars', readonly=True, tracking=True,
                              states={'draft': [('readonly', False)]})
    currency_id = fields.Many2one('res.currency', string='Currency', required=True, readonly=True,
                                  states={'draft': [('readonly', False)]},
                                  default=lambda self: self.env.company.currency_id.id)
    company_id = fields.Many2one('res.company', string='Company', readonly=True, tracking=True,
                                 states={'draft': [('readonly', False)]},
                                 default=lambda self: self.env['res.company']._company_default_get())
    active = fields.Boolean(tracking=True, default=True)
    maker_id = fields.Many2one('res.users', 'Maker', default=lambda self: self.env.user.id, tracking=True, copy=False)
    approver_id = fields.Many2one('res.users', 'Checker', tracking=True, copy=False)
    ...
    tds_amount = fields.Float('TDS Amount', readonly=True, compute='_compute_amount', copy=False, compute_sudo=True,
                              states={'draft': [('readonly', False)]})
    vat_amount = fields.Float('VAT Amount', readonly=True, copy=False, compute_sudo=True,
                              states={'draft': [('readonly', False)]})
    ...
    advance_amount_subtotal = fields.Monetary(string='Amount', readonly=True, compute='_compute_amount', compute_sudo=True, store=True)
    ...
    def _compute_total_deduction(self):
        for record in self:
            # print(record.vat_amount)
            record.total_deduction = record.security_deposit + record.tds_amount + 1
...
class VendorBillLine(models.Model):
    ...
    invoice_id = fields.Many2one('account.move', string="Bill No.", copy=False)
