...
class StockIssueReport(models.AbstractModel):
    ...
    _description = "Stock Issue Report"
    ...
    def get_report_data(self, data):
        ...
        sql = '''SELECT ipl.product_id,
                        ...
                        (SELECT array_to_string(array_agg(pv.name), ',')
                            FROM product_attribute_value_product_template_attribute_line_rel pr
		                    LEFT JOIN product_attribute_value pv
			                    ON pv.id = pr.product_attribute_value_id
                            WHERE pr.product_template_attribute_line_id = ipl.product_id
                            GROUP BY pr.product_template_attribute_line_id)  AS variant_name,
