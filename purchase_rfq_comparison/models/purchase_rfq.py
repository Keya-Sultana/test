...
class PurchaseRFQ(models.Model):
    ...
    rfq_date = fields.Date(string='Date of Request', default=datetime.now(), readonly=True,
                           tracking=True, required=True)
    ...
    def confirm_for_mail(self):
        res = self.env.ref('purchase_rfq_comparison.rfq_email_template_wizard')
    ...
    def print_rfq(self):
        ...
        vals = []
        for obj in self.purchase_rfq_lines:
            vals.append(({'product_id': obj.product_id.display_name,
                          'product_qty': obj.product_qty,
                          'product_uom_id': obj.product_uom_id.name,
                          }))
        data['vals'] = vals

        return self.env['report'].get_action(self, 'purchase_rfq_comparison.rfq_report', data=data)
...
class RFQProductLineWizard(models.Model):
    ...
    pr_line_ids = fields.Many2many('purchase.requisition.line', 'pr_rfq_line_rel', 'rfq_line_id', 'pr_line_id',
                                   'PR ids')
