...
class RFQEmailTemplateWizard(models.TransientModel):
    ...
    def _default_body_text(self):
        template = self.env.ref('purchase_rfq_comparison.purchase_rfq_email_template')
        return template.body_html

    subject = fields.Char('Subject', required=True)
    supplier_ids = fields.Many2many('res.partner', string='Supplier', required=True, domain=[('parent_id', '=', False), ('supplier_rank', '>', 0)])
