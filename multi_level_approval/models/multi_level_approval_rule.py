...
class ApprovalCategory(models.Model):
    _name = "multi.level.approval.rule"
    _description = "Multilevel Category Rule"
    ...
    active = fields.Boolean(string='Active', index=False, store=True, readonly=False, required=True,default=True
                            )
    ...
    approver_ids = fields.One2many('mla.rule.approver', 'category_id', string='Approvers', index=False, store=True,
                                   readonly=False)
    ...
    requirer_document = fields.Selection(CATEGORY_SELECTION, string='Documents', default="no", index=False,
                                         store=True, readonly=False)
    _sql_constraints = [
        ('model_company_uniq', 'unique(approval_model_id, company_id)',
         'The name of the model  must be unique per  company!'),
    ...
    def create(self, vals_list):
        ...
        approval_category = super().create(vals_list)
