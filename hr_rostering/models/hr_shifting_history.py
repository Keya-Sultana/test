...
class HrShiftingHistory(models.Model):
    ...
    effective_from = fields.Date(string='Effective Date', required=True, tracking=True)
    effective_end = fields.Date(string='Effective End Date', required=True, tracking=True)

    """ Relational Fields """
    employee_id = fields.Many2one("hr.employee", string='Employee Name', required=True, tracking=True)
    shift_id = fields.Many2one("resource.calendar", string="Shift Name", required=True, tracking=True,
                               domain="[('state', '=','approved' )]")

    """Shift Batch Relational Fields """
    shift_batch_id = fields.Many2one('hr.shift.employee.batch', string='Shift Batch', tracking=True)
    ...
    def _check_effective_from_validation(self):
        ...
        if get_previous_row_effective_to_date:
            ...
            if str_previous_effective_to:
                ...
                date_previous_effective_to = datetime.strptime(str(str_previous_effective_to), "%Y-%m-%d")
                ...
                if self.effective_from <= str_previous_effective_to:
                    raise ValidationError(
                        _("Current Effective date can not less then previous Effective date!! Already %s has a shift. "
                          "His effective end date %s") % (
                            emp_name, str_previous_effective_to))
                elif datetime.strptime(str(self.effective_from), "%Y-%m-%d") == ranged_last_date:
                    pass
                else:
                    raise ValidationError(
                        _("Current Effective date should not be too far from previous effective date!! %s effective "
                          "end date %s .") % (
                            emp_name, str_previous_effective_to))
