...
class HrManualAttendance(models.Model):
    ...
    sign_type = fields.Selection([
        ...
        ('sign_in', 'Sign In'),
        ('sign_out', 'Sign Out')
    ], string='Sign Type', tracking=True, required=True, default="both")
    ...
    def action_validate(self):
        ...
        for manual_attendance in self:
            ...
            vals1['operating_unit_id'] = manual_attendance.employee_id.default_operating_unit_id.id
            ...
            if manual_attendance.sign_type == 'both':
                ...
                if preAttData:
                    timeDiffInHrs = (self.getDateTimeFromStr(preAttData.check_out) - self.getDateTimeFromStr(
                        manual_attendance.check_in)).total_seconds() / 60 / 60
                    if timeDiffInHrs <= 15:
                ...
                if preAttData:
                    timeDiffInHrs = (self.getDateTimeFromStr(manual_attendance.check_out) - self.getDateTimeFromStr(
                        preAttData.check_in)).total_seconds() / 60 / 60
                    if timeDiffInHrs <= 15:
    ...
    def _check_value(self):
        ...
        if self.sign_type == "both":
            ...
            elif self.check_in > datetime.datetime.now():
                raise UserError(_("Check in time must be less than current time!"))
            elif deff > 13:
                raise UserError(
                    _("Difference between check in time and check out time duration must be upto 13 hours!"))
        elif self.sign_type == "sign_in":
            if self.check_in > datetime.datetime.now():
                raise UserError(_("Check in time must be less than current time!"))

        elif self.sign_type == "sign_out":
            if self.check_out > datetime.datetime.now():
    ...
    def _check_time(self):
        ...
        for h in self:
            ...
            if check_manual_att:
                ...
                date_time_check_in = datetime.datetime.strptime(h.check_in, "%Y-%m-%d %H:%M:%S") + datetime.timedelta(
                    hours=6)
                date_time_check_out = datetime.datetime.strptime(h.check_out, "%Y-%m-%d %H:%M:%S") + datetime.timedelta(
                    hours=6)
                raise ValidationError(
                    _("The duration of the period  (%s)  and  (%s)  are overlapping with existing Manual Attendance." % (
                    date_time_check_in, date_time_check_out)
                      ))
            ...
            if check_att:
                ...
                date_time_check_out = datetime.datetime.strptime(h.check_out, "%Y-%m-%d %H:%M:%S") + datetime.timedelta(
                    hours=6)
                raise ValidationError(
                    _("The duration of the period  (%s)  and  (%s)  are overlapping with existing Attendance." % (
                    date_time_check_in, date_time_check_out)
                      ))
