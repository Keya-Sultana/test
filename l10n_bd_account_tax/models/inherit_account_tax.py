...
class AccountTax(models.Model):
    ...
    name = fields.Char(tracking=True, string='Name')
    description = fields.Char(tracking=True)

    amount = fields.Float(tracking=True)

    type_tax_use = fields.Selection(tracking=True)
    amount_type = fields.Selection(_selection_amount_type, tracking=True)

    active = fields.Boolean(tracking=True)
    is_reverse = fields.Boolean(string='Reverse Accounting', tracking=True)
    is_vat = fields.Boolean(string='VAT Rule', tracking=True)
    is_tds = fields.Boolean(string='TDS Rule', tracking=True)
    price_include = fields.Boolean(tracking=True)
    include_base_amount = fields.Boolean(tracking=True)
    tax_adjustment = fields.Boolean(tracking=True)
    price_exclude = fields.Boolean(string='Excluded in Price', default=False, tracking=True)
    effect_base_price = fields.Boolean(string='Effect On Base Price', default=False, tracking=True)

    # account_id = fields.Many2one(tracking=True)
    # refund_account_id = fields.Many2one(tracking=True)
    ...
    mushok_amount = fields.Float(string='Mushok Value', digits=(16, 4), default=0.0, tracking=True,
                                 help='For Mushok-6.3')
    vds_amount = fields.Float(string='VDS Authority Value', digits=(16, 4), default=0.0, tracking=True,
                              help='For VDS Authority')

    include_in_expense = fields.Boolean(string='Include In Expense', tracking=True)
...
class AccountTaxSlabLine(models.Model):
    ...
    _description = 'Account TaxS lab Line'
    ...
    advance_id = fields.Many2one('vendor.advance', 'Advance')
