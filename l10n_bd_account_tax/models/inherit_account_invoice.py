...
class AccountInvoice(models.Model):
    ...
    amount_tds = fields.Float(string='TDS', compute='_compute_amount', store=True, readonly=True, copy=False, tracking=True)
    date_invoice = fields.Date(required=True, default=fields.Date.today())
    amount_vat_payable = fields.Float('VAT Payable', compute='_compute_amount', store=True, readonly=True, copy=False, tracking=True)
...
class AccountInvoiceLine(models.Model):
    ...
    def onchange_vat_id(self):
        if self.vat_id.id:
            self.tax_ids -= self.tax_ids.filtered(lambda x: x.is_vat is True)
            self.tax_ids += self.vat_id
        else:
            self.tax_ids -= self.tax_ids.filtered(lambda x: x.is_vat is True)
    ...
    def onchange_tds_id(self):
        if self.tds_id.id:
            self.tax_ids -= self.tax_ids.filtered(lambda x: x.is_tds is True)
            self.tax_ids += self.tds_id
        else:
            self.tax_ids -= self.tax_ids.filtered(lambda x: x.is_tds is True)
