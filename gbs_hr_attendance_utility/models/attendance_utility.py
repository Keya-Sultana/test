...
class AttendanceUtility(models.TransientModel):
    ...
    def getShiftLines(self, shiftId):
        ...
        for i, shiftLine in enumerate(shiftLines):
            shiftLinesMap[int(shiftLine[0])] = utility_class.ShiftLine(int(shiftLine[0]), shiftLine[1], shiftLine[2], shiftLine[3],
    ...
    def buildDutyTime(self, preStartDate, postEndDate, shiftList):
        ...
        while preStartDate <= postEndDate:
            for i, shift in enumerate(shiftList):
                effectiveDtStr = datetime.datetime(shift.effectiveDtStr.year, shift.effectiveDtStr.month, shift.effectiveDtStr.day)
                if effectiveDtStr <= preStartDate <= shift.effectiveDtEnd:
                    ...
                    if shiftObj:
                        ...
                        dutyTimeMap[preStartDate.strftime('%Y.%m.%d')] = utility_class.DutyTime(startDutyTime, endDutyTime,
                                                                                  shiftObj.isot,
                                                                                  otStartDutyTime, otEndDutyTime,
                                                                                  shiftObj.graceTime)
                        break
            preStartDate = preStartDate + day
        return dutyTimeMap
    ...
    def getPreviousDutyTime(self, date, dutyTimeMap):
        ...
        if previousDayDutyTime:
            ...
            previousDayDutyTime = utility_class.DutyTime(date.replace(hour=22, minute=00), date.replace(hour=23, minute=00), False, 0,
                                           0, 0)
            return previousDayDutyTime
    ...
    def getNextDutyTime(self, date, dutyTimeMap):
        ...
        if nextDutyTime:
            ...
            nextDutyTime = utility_class.DutyTime(date + timedelta(hours=23.9), date + timedelta(hours=23.95), False, 0, 0, 0)
            return nextDutyTime
