...
class HrEmpMobileBillLine(models.Model):
    ...
    bill_amount = fields.Float(string="Bill Amount", required=True, tracking=True,
                               readonly=True, states={'draft': [('readonly', False)]})
    amount = fields.Float(string="Exceed Amount", compute='_compute_amount', required=True, tracking=True, readonly=True, states={'draft': [('readonly', False)]})
    emp_mobile_phone = fields.Char('Mobile Number', readonly=True, tracking=True,
                                   states={'draft': [('readonly', False)]})

    """ Relational Fields """

    parent_id = fields.Many2one(comodel_name='hr.mobile.bill', ondelete='cascade', tracking=True,)
    employee_id = fields.Many2one('hr.employee', string="Employee", store=True, ondelete='cascade',
                                  compute='onchange_emp_mobile_phone', readonly=True, tracking=True)
    ...
    @api.depends('bill_amount', 'employee_id.current_bill_limit')
    def _compute_amount(self):
        for line in self:
            line.amount = (line.bill_amount - line.employee_id.current_bill_limit) if line.bill_amount > line.employee_id.current_bill_limit else 0
