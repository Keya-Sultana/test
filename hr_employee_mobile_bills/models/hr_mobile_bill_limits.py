...
class HrMobileBillLimits(models.Model):
    ...
    name = fields.Char('Name', required=True, tracking=True, states={'draft': [('invisible', False)],
                                                                     'applied': [('readonly', True)],
                                                                     'approved': [('readonly', True)]})
    effective_bill_date = fields.Date('Effective Date', required=True, tracking=True,
    ...
    line_ids = fields.One2many('hr.employee.mobile.bill.line', 'parent_id', "Line Ids", tracking=True,
                               states={'draft': [('invisible', False)],
