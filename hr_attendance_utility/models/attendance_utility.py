...
from . import utility_class as uc
...
class AttendanceUtility(models.TransientModel):
    ...
    _description = "Attendance Utility"
    # _auto = False
    ...
    def getDutyTimeByEmployeeId(self, employeeId, preStartDate, postEndDate):
        ...
        self._cr.execute(self.employee_shift_history_query,
                         (employeeId, preStartDate, postEndDate, preStartDate, postEndDate))
    ...
    def buildAlterDutyTime(self, startDate, endDate, employeeId):
        ...
        for i, alter in enumerate(alterLines):
            alterTimeMap[self.getStrFromDate(self.getDateFromStr(alter[0]))] = utility_class.DutyTime(
                self.getDateTimeFromStr(alter[1]),
    ...
    def buildAlterDutyTimeForDailyAtt(self, startDate, endDate, employeeId):
        ...
        alterTimeMap = {}
        for i, alter in enumerate(alterLines):
            alterTimeMap[self.getStrFromDate(self.getDateTimeFromStr(alter[1]).date())] = utility_class.DutyTime(
                self.getDateTimeFromStr(alter[1]),
                self.getDateTimeFromStr(alter[2]),
                alter[3], self.getDateTimeFromStr(alter[4]),
                self.getDateTimeFromStr(alter[5]), alter[6])
    ...
    def getAttendanceDataForDailyAtt(self, dutyTimeMap, employeeId, preStartDate, postEndDate):
        ...
        self._cr.execute(self.daily_attendance_query,
                         (employeeId, att_time_pre_end, att_time_post_end, att_time_post_start))
    ...
    def getAttendanceListByAlterDay(self, alterDayDutyTime, day, dutyTimeMap, employeeId):
        ...
        self._cr.execute(self.alter_attendance_query,
        ...
        for i, attendance in enumerate(attendance_line):
            ...
            attendanceDayList.append(uc.TempLateTime(att_check_in, att_check_out, duration))
        ...
        try:
            for i, calendar in enumerate(calendarList):
                if len(calendarList) == i + 1:
                    ...
                    shiftList.append(utility_class.Shift(calendarList[i][0], postEndDate,
                                                         calendarList[i][1], self.getShiftLines(calendarList[i][1])))
                else:
                    shiftList.append(
                        utility_class.Shift(calendarList[i][0],
                                            self.getDateFromStr(calendarList[i + 1][0]) - day,
                                            calendarList[i][1], self.getShiftLines(calendarList[i][1])))
    ...
    def getShiftLines(self, shiftId):
        ...
        for i, shiftLine in enumerate(shiftLines):
            shiftLinesMap[int(shiftLine[0])] = utility_class.ShiftLine(int(shiftLine[0]), shiftLine[1], shiftLine[2],
                                                                       shiftLine[3],
    ...
    def buildDutyTime(self, preStartDate, postEndDate, shiftList):
        ...
        while preStartDate <= postEndDate:
            for i, shift in enumerate(shiftList):
                effectiveDtStr = datetime.datetime(shift.effectiveDtStr.year, shift.effectiveDtStr.month,
                                                   shift.effectiveDtStr.day)
                if effectiveDtStr <= preStartDate <= shift.effectiveDtEnd:
                    ...
                    if shiftObj:
                        ...
                        dutyTimeMap[preStartDate.strftime('%Y.%m.%d')] = utility_class.DutyTime(startDutyTime,
                                                                                                endDutyTime,
                                                                                                shiftObj.isot,
                                                                                                otStartDutyTime,
                                                                                                otEndDutyTime,
        ...
        if check_in:
            ...
            if isLate is True:  # Check Absent OR Late
                ...
                if self.checkOnHolidays(employeeId, todayIsHoliday, compensatoryLeaveMap, oTMap) is True:
                    att_summary["holidays"].append(utility_class.Employee(employee))
                elif self.checkOnPersonalLeave(employeeId, currDate) is True:
                    att_summary["leave"].append(utility_class.Employee(employee))
                elif self.checkShortLeave(employeeId, self.convertDateTime(currentDaydutyTime.startDutyTime)) is True:
                    att_summary["short_leave"].append(utility_class.Employee(employee))
                elif isLate == True:
                    att_summary["late"].append(utility_class.Employee(employee, check_in))

            else:
                att_summary["on_time_present"].append(utility_class.Employee(employee, check_in))
            ...
            if self.checkOnHolidays(employeeId, todayIsHoliday, compensatoryLeaveMap, oTMap) is True:
                ...
                att_summary["absent"].append(utility_class.Employee(employee))
    ...
    def getPreviousDutyTime(self, date, dutyTimeMap):
        ...
        if previousDayDutyTime:
            ...
            previousDayDutyTime = utility_class.DutyTime(date.replace(hour=22, minute=00),
                                                         date.replace(hour=23, minute=00), False, 0,
                                                         0, 0)
            return previousDayDutyTime
    ...
    def getNextDutyTime(self, date, dutyTimeMap):
        ...
        if nextDutyTime:
            ...
            nextDutyTime = utility_class.DutyTime(date + timedelta(hours=23.9), date + timedelta(hours=23.95), False, 0,
                                                  0, 0)
            return nextDutyTime
    ...
    def getDateTimeFromStr(self, dateStr):
        if dateStr:
            return datetime.datetime.strptime(str(dateStr), "%Y-%m-%d %H:%M:%S")
        else:
            return None
    ...
    def checkOnPersonalLeave(self, employee, startDate):

        local_tz = pytz.timezone(employee.tz or self.env.user.partner_id.tz or 'GMT')
        local_dt = local_tz.localize(startDate, is_dst=None)
        utc_dt = local_dt.astimezone(pytz.utc)
        utc_dt = utc_dt.strftime("%Y-%m-%d %H:%M:%S")
        gmt_time = datetime.datetime.strptime(utc_dt, "%Y-%m-%d %H:%M:%S")

        query_str = """SELECT COUNT(h.id) FROM hr_leave h
                        LEFT JOIN hr_leave_type hhs ON hhs.id = h.holiday_status_id
        ...
        self._cr.execute(query_str, (employee.id, gmt_time))
    ...
    def getHolidaysByUnit(self, unitId, requested_date):
        holidays_query = """select t1.id,t1.date from hr_holidays_public_line as t1
                            join hr_holidays_public as t2 on t2.id=t1.year_id
    ...
    def getHolidaysByUnitDateRange(self, unitId, from_date, to_date):
        holidaysMap = False
        public_holiday_lines = self.env['hr.holidays.public.line'].search([('date', '>=', from_date),
                                                                           ('date', '<=', to_date),
                                                                           ('year_id.operating_unit_id', '=', unitId)])
        if public_holiday_lines:
            holidaysMap = {self.getStrFromDate2(line['date']): line['id'] for line in public_holiday_lines}
